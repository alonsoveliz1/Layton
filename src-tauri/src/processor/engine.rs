...
use super::flow::{FlowDirection, FlowKey, FlowRecord, FLOW_TIMEOUT_US};
            ...
            recv(packet_rx) -> msg => {
                if let Ok(pkt) = msg {
                    ...
                    if flow.should_terminate(pkt.timestamp, has_fin) {
                        ...
                        flow.calculate_derived_features();
                        flow.finalize_for_export();

                        // Get a copy to be sent to the classifier
            ...
            recv(expire_tick) -> _ => {
                ...
                flows.retain(|key, flow| {
                    ...
                    if !should_keep{
                        ...
                        // Add it to my vector of entries to be classified
