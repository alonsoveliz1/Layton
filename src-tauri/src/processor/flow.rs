...
impl FlowRecord {
    ...
    fn get_flow_direction(
        ...
        if self.key.ip_a == src_ip
            && self.key.ip_b == dst_ip
            && self.key.port_a == src_port
            && self.key.port_b == dst_port
        {
    ...
    pub fn update_subflow_features(
        &mut self,
        timestamp: u64,
            ...
            match direction {
                ...
                FlowDirection::Backward => {
                    if self.last_bwd_packet_timestamp > 0 {
                        ...
                        self.bwd_iat_min = self.bwd_iat_min.min(bwd_iat);
                        self.bwd_iat_max = self.bwd_iat_max.max(bwd_iat);
            ...
            if time_since_last > ACTIVITY_TIMEOUT_US {
                ...
                if self.is_in_active_period {
                    let active_duration = self
                        .last_activity_time
                        .saturating_sub(self.current_active_start);
        ...
        if self.fwd_consecutive_packets >= BULK_THRESHOLD {
            ...
            if self.fwd_bulk_duration > 0 {
                self.fwd_bulk_rate_avg =
                    self.fwd_bytes_curr_bulk / (self.fwd_bulk_duration as f64 / 1_000_000.0);
            }
        }
        ...
        if self.bwd_consecutive_packets >= BULK_THRESHOLD {
            ...
            if self.bwd_bulk_duration > 0 {
                self.bwd_bulk_rate_avg =
                    self.bwd_bytes_curr_bulk / (self.bwd_bulk_duration as f64 / 1_000_000.0);
            }
        }
    ...
    pub fn calculate_derived_features(&mut self) {
        ...
        if self.total_fwd_packets > 0 {
            self.fwd_segment_size_avg =
                (self.total_fwd_bytes as f64) / (self.total_fwd_packets as f64);
        }

        if self.total_bwd_packets > 0 {
            self.bwd_segment_size_avg =
                (self.total_bwd_bytes as f64) / (self.total_bwd_packets as f64);
        }
    ...
    pub fn finalize_for_export(&mut self) {
        // Convert u32::MAX and u64::MAX to 0 for uninitialized fields
        if self.fwd_packet_len_min == u32::MAX {
            self.fwd_packet_len_min = 0;
        }
        if self.bwd_packet_len_min == u32::MAX {
            self.bwd_packet_len_min = 0;
        }
        if self.packet_len_min == u32::MAX {
            self.packet_len_min = 0;
        }

        if self.flow_iat_min == u64::MAX {
            self.flow_iat_min = 0;
        }
        if self.fwd_iat_min == u64::MAX {
            self.fwd_iat_min = 0;
        }
        if self.bwd_iat_min == u64::MAX {
            self.bwd_iat_min = 0;
        }

        if self.active_min == u64::MAX {
            self.active_min = 0;
        }
        if self.idle_min == u64::MAX {
            self.idle_min = 0;
        }

        if self.fwd_seg_size_min.is_infinite() {
            self.fwd_seg_size_min = 0.0;
        }

        // Finalize any pending bulk transfers
        if self.fwd_consecutive_packets >= BULK_THRESHOLD {
            self.finalize_fwd_bulk();
        }
        if self.bwd_consecutive_packets >= BULK_THRESHOLD {
