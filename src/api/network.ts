...
function normalize_err(err: unknown): string {
...
export async function listNetworkDevices(): Promise<NetworkInterface[]> {
  try {
    ...
    const msg = normalize_err(e).toLowerCase();
    ...
    // Default
    throw new Error(`Failed to list network interfaces: ${normalize_err(e)}`);
  }
}
...
export async function startSystem(iface: string) { 
  return invoke("start_system", { interface: iface }); 
}

export async function stopSystem() { 
  return invoke("stop_system"); 
}

export async function startBaselineCollection(iface: string, path: string) { 
  return invoke("start_baseline_collection", { interface: iface, outputDir: path }); 
}

export async function stopBaselineCollection() { 
  return invoke("stop_baseline_collection"); 
}
